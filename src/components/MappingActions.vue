<template>
  <div class="mapping-actions">
    <div
      v-for="(question, i) in mappingQuestions"
      class="mapping-question"
      :key="question.id"
    >
      <div
        class="mapping-question__title"
        @click="e => setQuestionPopupActive(e, question.id)"
      >
        <div
          class="mapping-question__popup"
          v-if="activeQuestionPopup === question.id"
        >
          <div
            class="mapping-question__popup--backdrop"
            @click="setQuestionPopupInactive"
          ></div>
          <div class="mapping-question__popup--content">
            <div
              v-if="activeQuestionConfirm"
              class="mapping-question__popup--confirm"
            >
              <h3>Are you sure?</h3>
              <button
            ...
            <input
              ...
              @input="e => setQuestionTitle(e, question)"
            />
            ...
            <div class="mapping-question__popup--position">
              <div
                class="question-position--up"
                @click="() => decreaseQuestionPosition(question, i)"
              >
                <span class="material-icons">remove</span>
              </div>
              <div class="question-position">{{ question.position }}</div>
              <div
                class="question-position--down"
                @click="() => increaseQuestionPosition(question, i)"
              >
                <span class="material-icons">add</span>
              </div>
            </div>
            <button @click="activeQuestionConfirm = true">
              Delete question
            </button>
          </div>
        </div>
        <span class="material-icons">{{ getIcon(question.type) }}</span>
        {{ question.title }}
      </div>
      <div
        :class="{
          'mapping-question__tags--active': activeOptionPopup === question.id
        }"
        class="mapping-question__value mapping-question__tags"
        @click="e => setOptionPopupActive(e, question.id)"
      >
        <div
          class="mapping-question__tags--popup"
          v-if="activeOptionPopup === question.id"
        >
          <div
            @click="setOptionPopupInactive"
            class="mapping-question__popup--backdrop"
          ></div>
          <div class="mapping-question__tags--popup-content">
            <ul class="mapping-question__tags--list">
              <li
                @click="() => createOption(question, nextOptionColor)"
                class="mapping-question__tags--list-item"
                v-if="optionInput.length"
              >
                <div class="mapping-question__tags--create">Create:</div>
                <span
                  class="mapping-question__tags--tag"
                  :style="{ backgroundColor: nextOptionColor }"
                  >{{ optionInput }}</span
                >
              </li>
              <li
                v-for="option in question.MappingOptions.filter(
                  filterOptionList
                )"
                :key="option.id"
                class="mapping-question__tags--list-item"
        ...
        <div class="mapping-question__tags">
          <span
            v-for="option in currentItem.MappingOptions.filter(
              o => o.mappingQuestionId === question.id
            )"
            :key="option.id"
            class="mapping-question__tags--tag"
            :style="{ backgroundColor: option.color }"
            @click="() => removeRecordOption(option.id)"
          >
            {{ option.title }}
            <span
    ...
    <div class="mapping-question mapping-question--new">
      <div class="mapping-question__title" @click="newMappingQuestion">
        <span class="material-icons">add</span> New mapping question
      </div>
    </div>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async setRecordOption(data) {
      await this.addRecordMappingOption(data);
      this.optionInput = "";
    },
    async removeRecordOption(data) {
      if (this.activeOptionPopup !== null) {
        await this.removeRecordMappingOption(data);
      }
    },
    getOptionColor() {
      const colors = [
...
.mapping-actions {
  ...
  .mapping-question {
    ...
    &--new {
      ...
      .mapping-question__title,
      .mapping-question__title .material-icons {
