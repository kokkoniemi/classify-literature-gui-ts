<template>
  <section id="classifier" v-if="currentItem">
    <h4 class="statusbar">
      ...
      modified: {{ modifiedFormatted }} | status:
      {{ currentItem.status }} |&nbsp; db:
      {{ currentItem.databases.join(", ") }}
    </h4>
    ...
    <p class="publication">
      ...
      <span v-if="currentItem.Publication"
        >{{ currentItem.Publication.name }}, jufo-level:
    ...
    <div
      ...
      :settings="{}"
      :style="{ paddingBottom: abstractPaddingBottom }"
    >
        ...
        <div class="bottom-bar__actions">
          ...
          <div class="inclusion-actions" v-if="tab === 'inc-exc'">
            ...
            <button
              @click="setUncertain"
              :class="[
                currentItem.status === 'uncertain' && 'action--selected'
              ]"
...
export default Vue.extend({
  ...
  created() {
    window.addEventListener("keydown", (this as any).moveTo);
  },
  destroyed() {
    window.removeEventListener("keydown", (this as any).moveTo);
  },
  methods: {
    ...mapActions([
      "setItemStatus",
    ...
    async setExcluded() {
      await this.$store.dispatch("setItemStatus", "excluded");
      this.$store.dispatch("setNextItem", this.nextFlag);
    },
    async setUncertain() {
      await this.$store.dispatch("setItemStatus", "uncertain");
      this.$store.dispatch("setNextItem", this.nextFlag);
    },
    async setIncluded() {
      await this.$store.dispatch("setItemStatus", "included");
      this.$store.dispatch("setNextItem", this.nextFlag);
    },
    async setComment(e: any) {
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      const c = this;
      await debounce(
        async () => await c.$store.dispatch("setItemComment", e.target.value),
        1000
      );
    },
    async setNextItem(skip: boolean): Promise<void> {
      if (skip) {
        return;
      }
      const index = this.pageItems.findIndex(
        (item: Record) => item.id === this.currentItem.id
      );
      if (index >= this.pageLength - 1) {
        this.$store.dispatch("setPage", this.page + 1);
      } else {
        this.$store.dispatch("setCurrentItem", this.pageItems[index + 1]);
      }
    },
    async setPrevItem(): Promise<void> {
      const index = this.pageItems.findIndex(
        (item: Record) => item.id === this.currentItem.id
      );
      if (index <= 0 && this.page > 1) {
        await this.$store.dispatch("setPage", this.page - 1);
        this.$store.dispatch(
          "setCurrentItem",
          this.pageItems[this.pageItems.length - 1]
        );
      } else if (index > 0) {
        this.$store.dispatch("setCurrentItem", this.pageItems[index - 1]);
      }
    },
    nltobr(str: string): string {
      return str.replace(/(?:\r\n|\r|\n)/g, "<br>");
    },
    sanitizeAbstract(str: string): string {
      ...
      if (!this.moveLock) {
        switch (e.keyCode) {
          case keyCodes.ARROW_LEFT:
            (this as any).setPrevItem();
            break;
          case keyCodes.ARROW_RIGHT:
            (this as any).setNextItem();
            break;
          default:
        }
      }
